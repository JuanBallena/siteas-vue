<template>
  <div>
    <ValidationObserver ref="datosBuscarUsuario">
      <v-row>
        <v-col cols="12" md="5">
...
export default {
  ...
  methods: {
    ...
    ...mapActions('usuarioxCentro', [
      'getUsuarioxCentroData',
      'changeStateUsuarioCentro',
      'setUsuarioCentroData',
    ]),
    ...
    /*filterUsuariosxCentro() {
      const items = this.usuarioxCentroData.filter(usuarioxCentro => usuarioxCentro.rol.idRol != ROL_ADMINISTRADOR && usuarioxCentro.rol.idRol != ROL_SUPERVISOR)
      if (items.length == 0) {
        this.setPages(null)
      }
      this.setUsuarioCentroData(items)
    },*/

    async cambiarEstadoUsuario(item, idEstadoUsuarioCentro, descripcionEstadoUsuarioCentro) {
      const res = await swal.confirm('¿Está seguro de cambiar el estado de este usuario?')
      if(res) {
        const params = {
          usuario: {
            data: {
        ...
        await this.update({ idUsuario: item.usuario.idUsuario, params: params })
        if (this.isSuccess) {
    ...
    async buscarUsuario() {
      const isValid = await this.$refs.datosBuscarUsuario.validate()
      if (isValid) {
        this.textToSearchAux = this.textToSearch || ''
        const paramsUrlUsuario = `?page=${0}&size=${this.itemsPerPage}&usuario=${this.textToSearchAux}&idCentroAsistencial=${this.idCentroAsistencial}&notAdmin=true`
        await this.getUsuarioxCentroData(paramsUrlUsuario)
        //this.filterUsuariosxCentro()
    ...
    async onChangePage() {
      const paramsUrlUsuario = `?page=${this.pageApi}&size=${this.itemsPerPage}&usuario=${this.textToSearchAux}&idCentroAsistencial=${this.idCentroAsistencial}&notAdmin=true`
      await this.getUsuarioxCentroData(paramsUrlUsuario)
      //this.filterUsuariosxCentro()
    }
  }
